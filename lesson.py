...
with socket.socket() as socket:
	socket.bind((host,port))	 #ip ve portu gir.
	socket.listen()			#dinlemeye başla.
	connection, address = socket.accept()	 #gelen istekleri dinlemeye al.
	with connection:	#bağlantı varsa,
		print("Gelen bağlantı:", address)
		while True:
			data = connection.recv(1024)	#sürekli 1024 byte veri al.
			if not data:
				break #veri yoksa kapat.
			connection.sendall(data)	 #tekrar paket almaya başla.
...
with socket.socket() as socket:
	socket.connect((host,port))		 #bağlantı yap.
	socket.sendall(b"hello")
	data = socket.recv(1024)	#kullanıcıdan gelen veri.
print(data)
